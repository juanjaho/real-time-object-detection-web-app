...
export async function runYoloModel(preprocessedData: any): Promise<[any, number]> {
  
  // Create session and set options. See the docs here for more options: 
  //https://onnxruntime.ai/docs/api/js/interfaces/InferenceSession.SessionOptions.html#graphOptimizationLevel
  const session = await ort.InferenceSession
                          .create('./_next/static/chunks/pages/yolov7-tiny.onnx', 
...
export function imagenetClassesTopK(classProbabilities: any, k = 5) {
  ...
  const topK = _.take(sorted, k).map((probIndex: Array<number>) => {
    const iClass = yoloClasses[probIndex[1]];
    return {
      id: iClass[0],
