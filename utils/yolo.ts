...
export async function postprocess(outputTensor: Tensor, numClasses: number) {
  ...
  classesIndxArr.forEach((classIndx, i) => {
    ...
    const className = yoloClasses[classIndx];
...
export function non_max_suppression(
  ...
  // NOTE: This is a bit of a hack, but it seems that the above sort is causing a bug 
  // where .sort() does not accept number[] as a type. This is a workaround.
